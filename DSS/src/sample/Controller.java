...
public class Controller {
	...
	public ClassifierObject StartTrainAndTest() throws Exception {
		...
		allClassifiers[0]
				.setScore(Evaluater(allClassifiers[0].getCl(), nominalTrainAndTestIns[0], nominalTrainAndTestIns[1]));

		allClassifiers[1] = new ClassifierObject(classifiers.DecisionTree(nominalTrainAndTestIns[0]), "Decision Tree");
		allClassifiers[1]
				.setScore(Evaluater(allClassifiers[1].getCl(), nominalTrainAndTestIns[0], nominalTrainAndTestIns[1]));

		allClassifiers[2] = new ClassifierObject(classifiers.KNearestNeighbour(3, numericTrainAndTestIns[0]),
				"K Nearest Neighbour");
		allClassifiers[2]
				.setScore(Evaluater(allClassifiers[2].getCl(), numericTrainAndTestIns[0], numericTrainAndTestIns[1]));

		allClassifiers[3] = new ClassifierObject(classifiers.ArtificalNeuralNetwork(numericTrainAndTestIns[0]),
				"Artifical Neural Network");
		allClassifiers[3]
				.setScore(Evaluater(allClassifiers[3].getCl(), numericTrainAndTestIns[0], numericTrainAndTestIns[1]));

		allClassifiers[4] = new ClassifierObject(classifiers.SupportVectorMachine(numericTrainAndTestIns[0]),
				"Support Vector Machine");
		allClassifiers[4]
				.setScore(Evaluater(allClassifiers[4].getCl(), numericTrainAndTestIns[0], numericTrainAndTestIns[1]));
